...
### 2026-09-22 세션 — 주 시리즈 확장 재개

**1. 판단: 열어둔 "잔여 16개 주 확장 재검토" 근거가 충족됐다**
- GSC 클릭 26개 중 **18개(69%)가 주 시리즈** — 60%(09-14) → 69%로 계속 상승. Georgia가 첫 클릭.
- **이번 주 주 페이지 3개(CA·MO·OR)가 Bing에서도 첫 클릭** — 이전까지 Bing 클릭은 전부 tax·hourly였다.
- GA에서 **Google organic이 2주 연속 Bing 추월**(14 vs 7).
→ 주 시리즈는 **두 엔진 모두에서 클릭이 나는 유일한 클러스터**다. 누락 16개 중 인구 최대 2개(**Connecticut 3.68M, Utah 3.5M**)를 선정. 나머지 14개는 전부 2M 이하(Kansas 2.95M, Mississippi 2.94M, Idaho 2.0M ...).

**2. 신규: `late-fee-laws-freelancers-connecticut.html`**
- **§37-4 12% 고리대금 상한은 "대출"용이다.** Justia 판례 주석: 신용판매엔 부적용(3 Conn. App. 306), **대출이 아닌 서비스 구매 채무엔 부적용**(53 Conn. App. 455). 프리랜서 서비스 인보이스엔 헤드라인보다 훨씬 약한 제약. 단 명문 예외가 아니라 판례 기반이라 **보수적으로 1%/월(12%/년) 권고**, 미니 계산기 기본값도 1%.
- **차별화 포인트 §37-3a**: 연체료 조항이 **없어도** 법원이 "지급기일 후 금전 지체에 대한 손해"로 **연 10%까지** 인정 가능, **대출 외 채무**(=인보이스)에 적용. 재량이지만 DiLieto v. County Obstetrics 판결로 "본안 승소만으로 부당 지체 성립" — 문턱 낮음. §37-1은 합의 없을 때 8%.
- 소액소송 **$5,000**(§51-15(d), 주택개량·신축만 $15,000), 수수료 $95, 법원이 우편 송달. 소멸시효 6년(§52-576).
- 비교표에 MA의 **c.231 §6C(계약 채무 12%, 조항 유무 무관)**를 CT §37-3a와 대비 — 같은 방향의 제도라 좋은 비교점.

**3. 신규: `late-fee-laws-freelancers-utah.html`**
- **§15-1-1: 2019 개정으로 "서비스 계약"과 "서면·구두·묵시적 계약"을 명문으로 포함.** 당사자 합의 시 **어떤 이율이든** 가능. 미합의 시 **연 10%**. 대출 중심 고리대금법에서 유추해야 하는 대부분의 주와 달리 Utah는 문제 자체를 없앴다.
- **"연 10%이지 월 10%가 아니다"** — 계약에 이율이 없는데 lien에 "월 10%"(=연 120%)를 청구한 실제 분쟁 사례로 짚음. 일일 연체료 페이지와 연결.
- **소액소송 $20,000** — **1차 법령 §78A-8-102(2022-05-04 시행)** 기준: 2025-01-01~2029-12-31 $20,000, **2030-01-01부터 $25,000** 단계 인상. 소멸시효 서면 6년(§78B-2-309), 구두 4년(§78B-2-307). 판결 후 이자 = 연방 판결후이율 + 2%(§15-1-4).

**4. 오류를 사전에 잡은 것 2건 (기록 가치 있음)**
- **Utah 소액소송**: Taylorsville 시 법원 공식 페이지가 **$15,000**이라고 했다 — 2024년까지의 낡은 수치. 1차 법령을 확인해서 $20,000으로 잡았다. **정부 사이트라도 2차 출처다. 법령 원문(le.utah.gov 등)을 확인할 것.** 페이지 본문에도 "오래된 출처는 $15,000으로 표시됨"을 명시해 독자 혼란을 선제 차단.
- **비교표 NY "9%/year statutory"**: 작성 중 넣었다가 **이번 세션 미검증이고 우리 NY 페이지에도 없는 수치**임을 확인하고 "Contract-dependent"(허브 표의 검증값)로 교체. 소멸시효 열도 인접주 다수가 이번 세션 미검증이라 **아예 제외**했다. **비교표는 과거 세션 검증값만 재사용할 것.**

**5. 일관성 갱신 — 36개 주로 전환**
- 위젯 5개(`how-to-negotiate`, `is-a-daily-late-fee-legal`, `client-discovery-call-questions`, `retainer-client-not-paying`, `is-it-worth-suing`)에 CT·UT 추가. 전부 옵션 36개 확인.
- **UT 표시값에 금액이 2개**(`$20,000 (to $25,000 in 2030)`) — **08-11 NY/IN 파싱 버그와 동일 유형.** 경계값 9케이스로 검증: UT $19,000 "적합" / **$22,000 "초과"** / CT $4,500 "적합" / $6,000 "초과" — 두 번째 금액을 잘못 읽지 않음. `how-to-negotiate`는 08-11에 고친 첫 금액 추출 정규식 덕에 안전, 나머지 3개는 숫자 필드 `sc`로 비교해 표시 문자열과 무관.
- "34 states" 텍스트 **11곳 → 36**, 허브 `late-payment-fee.html` 비교표 행 + 주 카드 2개(주 링크 36개).
- **다음에 주를 추가할 때 체크리스트**: ① 신규 주 페이지 ② 위젯 5개 데이터 ③ "N states" 텍스트 전수 치환 ④ 허브 표 행 + 카드 ⑤ blog/index ⑥ sitemap ⑦ 경계값 테스트. 하나라도 빠지면 위젯에서 "36개 주 커버"라고 하면서 선택지엔 없는 불일치가 생긴다.

**6. 보강: `email-templates/sending-scope-of-work.html`** — 이번 주 최대 신규 신호
- Bing **2 → 20노출 2.15위, 0클릭.** 쿼리가 전부 B2B·산업용: `professional email template scope of work oem vendor`(2.75위), `...clarification supply chain executive`(1위), `how to email attached is the scope of work for egg grading... we don't have scope of work yet`(1위). 페이지에 vendor·supplier·clarification 언급 **0회**, 619단어.
- **08-17 이후 3번째 "비프리랜서 독자" 독립 신호**(08-17 vendor/painter, 이번 OEM/공급망/양계 설비). 이 사이트 독자의 상당수는 프리랜서가 아니라 **B2B 거래 실무자**일 가능성이 점점 높다.
- 템플릿 3종 추가: 공급사 견적 요청 / 범위 명확화 요청 / SOW가 아직 없을 때. FAQ 4→6 + **FAQPage 스키마 신설**(원래 없었음).
- **함정 발견**: 기존 `copyTemplate()`이 `querySelector('.copy-btn')`·`getElementById('templateContent')`로 **첫 템플릿에 하드코딩**돼 있었다. 템플릿을 추가하면 **어느 Copy 버튼을 눌러도 1번만 복사**된다. 범용 `copyTpl(btn)`(가장 가까운 `.template-box` 기준) 신설 + 기존 함수 선택자를 `#templateBox .copy-btn`으로 고정. jsdom 검증: 4개 버튼 각자 자기 템플릿 복사, 제목 4/4 고유. **다른 이메일 템플릿 페이지에 템플릿을 추가할 때도 같은 함정이 있을 것이다.**

**7. 추적 결과**
- `freelance-tax-guide-for-beginners` title 변경(09-14): Bing 48노출 7.0위 → **63노출 6.79위**, 여전히 0클릭. 미세 개선이나 **1주차라 판정 보류**(10-05 전후 판정).
- `milestone-payment-schedule-long-projects`(09-07): 아직 양 엔진 미등장. 2주차, 판정 보류.
- `client-discovery-call-questions`: Bing 1→**3노출 2.67위**로 성장. `invoicing-software-automatic-late-fees`가 Bing 첫 등장(1노출 1위).

**8. 데이터**: Bing 노출 249→**349**(+40%), 클릭 13→**18**(6주 연속 성장). GSC 클릭 25→26. GA MAU 78→74, **google 14 / bing 7**.
**제휴 → 구간 A 유지**: MAU 74(<500) / 상업 페이지 월 조회 1 미만 / 미국 비중 미달. 언급 안 함.
